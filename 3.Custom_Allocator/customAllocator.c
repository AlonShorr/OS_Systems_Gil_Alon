...
bool is_valid_malloc_ptr (void *ptr) {
    if (cm) printf("  [is_valid_malloc_ptr] ptr: %p\n", ptr);
    Block* current = heap_blocks_head;
    if(current == NULL){
        if (cm) printf("  [is_valid_malloc_ptr] heap_blocks_head is NULL\n");
        return false; //NULL check inside the functions
    } 

    while (current) {
        if (current == ptr){ // TODO: Conversion between malloc and linkedlist ptr
            if (cm) printf("  [is_valid_malloc_ptr] found valid malloc pointer: %p\n", ptr);
            return true;
        }
            current = current->next;
    }
    if (cm) printf("  [is_valid_malloc_ptr] pointer not found in heap_blocks_head\n");
    return false;
}
*/
...
void* customMalloc(size_t size){
    if (cm) printf("  [customMalloc] size: %zu\n", size);
    if (size <= 0) {
        fprintf(stderr, "<customMalloc error>: passed nonpositive size\n"); // this print is for us to debug
        return NULL;
    }
    if (heap_blocks_head == NULL) {
        if (cm) printf("  [customMalloc] heap_blocks_head is NULL, initializing heap\n");
        heap_start = sbrk(0);
        ...
        if (cm) printf("  [customMalloc] created first block, returning pointer\n");
        return (char*)heap_blocks_head + sizeof(Block);
    }

    if (cm) printf("  [customMalloc] heap_blocks_head is not NULL, searching for free block\n");
    ...
    if (block == NULL) {
        if (cm) printf("  [customMalloc] no free block found, requesting more memory\n");
    ...
    if (cm) printf("  [customMalloc] found or created a block, returning pointer\n");
    return (char*)block + sizeof(Block);
}

void customFree(void* ptr) {
    if (cm) printf("  [customFree] ptr: %p\n", ptr);
